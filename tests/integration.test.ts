...
describe("Integration Tests", () => {
    describe("Complete Workflow", () => {
        ...
        beforeEach(() => {
            ...
            const tools = [
                {
                    name: "safe-tool",
                    description: JSON.stringify({ desc: "A safe tool", version: "1.0.0" }),
                ...
                {
                    name: "another-tool",
                    description: JSON.stringify({ desc: "Another safe tool", category: "utility" }),
        ...
        it("should successfully validate a complete valid manifest", async () => {
            ...
            await expect(validateManifest(loadedManifest, validRegistry, policyManager))
        ...
        it("should reject manifest with policy violations", async () => {
            const maliciousManifest: McpManifest = {
                ...
                tools: [
                    {
                        name: "malicious-tool",
                        description: "This tool uses fs.readFileSync to read sensitive files",
                        schema: z.string(),
            ...
            const loadedManifest = await loadManifest("./test-manifest.json");
            await expect(validateManifest(loadedManifest, maliciousRegistry, policyManager))
        ...
        it("should reject manifest with exec violations", async () => {
            ...
            const loadedManifest = await loadManifest("./test-manifest.json");
            await expect(validateManifest(loadedManifest, execRegistry, policyManager))
                .rejects.toThrow(/policy veto/);
        });
        ...
        it("should reject manifest with network violations", async () => {
            ...
            const loadedManifest = await loadManifest("./test-manifest.json");
            await expect(validateManifest(loadedManifest, networkRegistry, policyManager))
                .rejects.toThrow(/policy veto/);
        });
        ...
        it("should allow manifest with trusted network domains", async () => {
                ...
                tools: [
                    {
                        ...
                        description: JSON.stringify({ desc: "This tool fetches from https://example.com/api" }),
                        schema: z.string(),
            ...
            mockedFs.readFile.mockResolvedValue(JSON.stringify(trustedNetworkManifest));
            ...
            const loadedManifest = await loadManifest("./test-manifest.json");
            await expect(validateManifest(loadedManifest, trustedNetworkRegistry, policyManager))
                .resolves.not.toThrow();
        });
    });
    ...
    describe("Custom Policy Integration", () => {
        it("should work with custom policy rules", async () => {
            ...
            await expect(validateManifest(manifest, registry, policyManager))
                .rejects.toThrow(/policy veto/);
        });

        it("should work with custom prompt shields", async () => {
            const customShield = async (description: string): Promise<boolean> => {
                return !description.includes("shielded-content");
            };

            const policyManager = new PolicyManager();
            policyManager.registerPolicies(builtInPolicies);
            policyManager.registerShields([customShield]);
            ...
            const manifest: McpManifest = {
                ...
                tools: [
                    {
                        name: "shielded-tool",
                        description: JSON.stringify({ desc: "This tool contains shielded-content" }),
                        schema: z.string(),
            ...
            await expect(validateManifest(manifest, registry, policyManager))
                .rejects.toThrow(/prompt shield veto/);
        });
    });
