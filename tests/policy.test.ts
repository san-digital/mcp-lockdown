...
describe("PolicyManager with refined policies and shields", () => {
    const safeTool: McpTool = { name: "safe-tool", description: JSON.stringify({ desc: "valid" }), inputSchema: z.any(), handlerHash: "", publicKey: "" };
    const fsTool: McpTool = { name: "fs-tool", description: "uses fs.readFileSync", inputSchema: z.any(), handlerHash: "", publicKey: "" };
    const execTool: McpTool = { name: "exec-tool", description: "child_process.exec('rm -rf /')", inputSchema: z.any(), handlerHash: "", publicKey: "" };
    const evalTool: McpTool = { name: "eval-tool", description: "eval('2+2')", inputSchema: z.any(), handlerHash: "", publicKey: "" };
    const netTool: McpTool = { name: "net-tool", description: "fetch('https://malicious.com')", inputSchema: z.any(), handlerHash: "", publicKey: "" };
    const longTool: McpTool = { name: "long-tool", description: "a".repeat(201), inputSchema: z.any(), handlerHash: "", publicKey: "" };
    const hiddenTool: McpTool = { name: "hidden-tool", description: "---\nsecret", inputSchema: z.any(), handlerHash: "", publicKey: "" };
    const nameTool: McpTool = { name: "BadName!", description: JSON.stringify({ desc: "ok" }), inputSchema: z.any(), handlerHash: "", publicKey: "" };
    const zeroWidthTool: McpTool = { name: "zw-tool", description: "hello\u200Bworld", inputSchema: z.any(), handlerHash: "", publicKey: "" };
    const badJsonTool: McpTool = { name: "json-tool", description: "not a json", inputSchema: z.any(), handlerHash: "", publicKey: "" };
